async function SubmitPosition(e) {
    ...
    const redirectUrl = `${BaseUrl}position`;
    ...
    submitBtn.innerHTML = `Adding Position ...`;
...
// return;
    ...
    try {
        ...
        let url = getSubmitAPI({ resource: "position", id: sportID })
...
async function fetchPosition(positionID) {
    const apiUrl = `${BaseUrl}position/details/${positionID}`;
...
function displayPositionInfo(data) {
    ...
    if (Object.keys(positionDetail).length > 0) {
        populateFormFields(positionDetail);
    }
