...
class App_Controller extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        if ($loggedInUser) {
            $this->full_name = ($loggedInUser->first_name ?? '') . ' ' . ($loggedInUser->last_name ?? '');
        } else {
            $this->full_name = 'Guest'; // Default name for non-authenticated users
        }
        $this->load->vars('full_name', $this->full_name);
    ...
    protected function isAuthorized()
    {
        ...
        $this->db->select('t.token, t.expiry, t.user_id');
        $this->db->from('authtokens t');
    ...
    protected function isUserAuthenticated()
    {
        ...
        if (!$auth_token) {
            ...
            if ($recreated_hash === $token_hash) {
                ...
                $user_details = $this->Admin_model->validate_user($user_id);
                if (!$user_details) {
